...
function Highlight<T extends ElementType = 'div'>({ ref, ...props }: HighlightProps<T>) {
  const {
    as: Component = 'div',
    children,
    value,
...
function HighlightItem<T extends ElementType>({
  ...
  const itemTransition = (reduceMotion ? { duration: 0 } : transition) ?? contextTransition;
          ...
          <AnimatePresence initial={false} mode="wait">
            {isActive && !isDisabled && (
              <motion.div
                ...
                transition={reduceMotion ? { duration: 0 } : itemTransition}
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{
                  opacity: 0,
